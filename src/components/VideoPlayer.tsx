...
export const VideoPlayer: React.FC<VideoPlayerProps> = ({
  ...
  return (
    <div className="bg-[#0b1626] border border-yellow-950/80 rounded-2xl overflow-hidden shadow-2xl shadow-black/50">
      ...
      <div className="relative aspect-video w-full bg-[#02060d] overflow-hidden group">
        ...
        <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/90 to-transparent p-3 flex items-center justify-between text-xs text-slate-300 z-20">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={cycleSpeed}
              className="px-2 py-0.5 rounded bg-slate-800/80 hover:bg-slate-700 text-[11px] font-mono font-bold text-yellow-500 border border-slate-700 transition"
      ...
      <div className="p-5 sm:p-6 border-b border-yellow-950/60 bg-[#081220]/70 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            <span className="text-[11px] font-bold uppercase tracking-wider text-yellow-500 bg-yellow-950/60 border border-yellow-800/50 px-2 py-0.5 rounded">
      ...
      <div className="p-5 sm:p-6 space-y-5">
        ...
        {/* Framework Box */}
        <div className="p-4 rounded-xl bg-gradient-to-r from-yellow-950/40 via-yellow-950/30 to-yellow-950/20 border-l-4 border-yellow-500 border-y border-r border-yellow-900/40">
        ...
        <div>
          ...
          <ul className="grid gap-2.5">
            {(lesson.keyPoints || lesson.summary).map((point: string, i: number) => (
              <li
                key={i}
                className="flex items-start gap-2.5 text-xs sm:text-sm text-slate-300 bg-[#07111e] p-3 rounded-lg border border-yellow-950/70"
              >
                <span className="w-5 h-5 rounded-full bg-yellow-950 border border-yellow-800 text-yellow-500 flex items-center justify-center font-mono text-[11px] font-bold flex-shrink-0 mt-0.5">
                  {i + 1}
                </span>
